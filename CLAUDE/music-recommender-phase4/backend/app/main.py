...
from app.routers import bandit_admin, experiments, explainability
...
async def lifespan(app: FastAPI):
    logger.info("Starting Music Recommender API (Phase 4)...")
    await create_tables()
    ...
    # Phase 4: load experiments from Redis on startup
...
app = FastAPI(
    ...
    description=(
        ...
        "Thompson Sampling · ADWIN/KS drift · SPRT A/B testing · SHAP explanations"
    ),
    lifespan=lifespan,
)
...
app.include_router(experiments.router)       # Phase 4
app.include_router(explainability.router)    # Phase 4
...
async def health():
    ...
    return HealthResponse(
        status=overall, db=db_status, redis=redis_status, version="4.0.0"
    )
