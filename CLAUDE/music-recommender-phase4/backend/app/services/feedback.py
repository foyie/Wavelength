...
-------------------------
Extends Phase 3 with automatic experiment observation recording.
When running experiments are active, every feedback event is routed
to the appropriate experiment arm and recorded.
"""
import logging
...
from app.models import User, Song, Interaction, PlaylistItem, ExperimentObservation
...
async def process_feedback(
    ...
    # Phase 4: record observation for all running experiments
    running_experiments = experiment_registry.list_running()
    for exp in running_experiments:
        arm = experiment_registry.assign_arm(user_id, exp.id)
        if arm:
            await experiment_registry.record_observation(exp.id, user_id, max(0.0, reward))
            db.add(ExperimentObservation(
    ...
    logger.info(
        f"Feedback: user={user_id} song={song_id} action={action} reward={reward} "
        f"kafka={'ok' if published else 'fallback'} experiments={len(running_experiments)}"
    )
