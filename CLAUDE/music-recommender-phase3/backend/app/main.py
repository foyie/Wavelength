...
async def lifespan(app: FastAPI):
    logger.info("Starting Music Recommender API (Phase 3)...")
    await create_tables()
    ...
    # Phase 3: start background metrics collector
...
app = FastAPI(
    ...
    description=(
        ...
        "Thompson Sampling bandit + ADWIN/KS drift detection"
    ),
    lifespan=lifespan,
)
...
app.include_router(monitoring.router)   # Phase 3
...
async def health():
    ...
    return HealthResponse(
        status=overall, db=db_status, redis=redis_status, version="3.0.0"
    )
